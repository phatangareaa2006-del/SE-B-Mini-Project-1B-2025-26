...
const getMockUser = (email: string, fullName: string, role: string) => {
  return {
    id: uuidv4(),
    email,
    fullName,
...
const createDemoProfile = (userId: string, email: string, fullName: string) => {
  const profile = {
    id: uuidv4(),
    user_id: userId,
    email,
  ...
  try {
    ...
    try {
      ...
      await client.query(
        ...
        [uuidv4(), userId, role, new Date()]
      );

      // Create profile
      await client.query(
        'INSERT INTO profiles (id, user_id, email, full_name, college_id, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7)',
        [uuidv4(), userId, email, fullName, collegeId || null, new Date(), new Date()]
      );
