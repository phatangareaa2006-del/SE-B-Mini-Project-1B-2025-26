...
router.get('/', authMiddleware, async (req, res) => {
    try {
        ...
        queryStr += ` ORDER BY created_at DESC LIMIT ${Math.min(parseInt(limit) || 50, 100)}`;
...
router.put('/:id/read', authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const result = await query('UPDATE notifications SET is_read = true WHERE id = $1 AND user_id = $2 RETURNING *', [id, req.userId]);
...
router.get('/unread/count', authMiddleware, async (req, res) => {
    try {
        const result = await query('SELECT COUNT(*) as unread_count FROM notifications WHERE user_id = $1 AND is_read = false', [req.userId]);
